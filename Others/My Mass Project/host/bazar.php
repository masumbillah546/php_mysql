...
<!----------------------------------------------------------------------------------->
	...
	echo "<a href='apple_bazar.php'>Apple's Bazar inserted successfully!!</a>";


}elseif($_POST['select']=="2"){	
	$sql= "INSERT INTO masum(bazar,price) VALUES ('$bazar[$i]','$price[$i]')";
	mysqli_query($con, $sql);
	echo "<a href='masum_bazar.php'>Masum's Bazar inserted successfully!!</a>";

}elseif($_POST['select']=="3"){	
	$sql= "INSERT INTO tanvir(bazar,price) VALUES ('$bazar[$i]','$price[$i]')";	
	mysqli_query($con, $sql);
	echo "<a href='tanvir_bazar.php'>Tanvir's Bazar inserted successfully!!</a>";

}elseif($_POST['select']=="4"){
	$sql= "INSERT INTO antor(bazar,price) VALUES ('$bazar[$i]','$price[$i]')";
	mysqli_query($con, $sql);
	echo "<a href='antor_bazar.php'>Antor's Bazar inserted successfully!!</a>";

}else{return false;

// echo "<a href='bazar.php'>Your Bazar alrady inserted</a>";
